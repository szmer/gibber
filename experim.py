...
class ResultCategory(object):
    ...
    def print_scores(self, num_all, num_all_unique):
        print('All: {}, good: {}, accuracy: {}'.format(num_all, self.good_some_ngbs, self.good_some_ngbs/num_all))
        print('Unique: {}, good: {}, accuracy: {}'.format(num_all_unique, self.unique_good_some_ngbs, self.unique_good_some_ngbs/num_all_unique))
        print('With full ngb coverage: {}, good: {}, accuracy: {}'.format(self.num_all_ngbs, self.good_all_ngbs, self.good_all_ngbs/self.num_all_ngbs))
        print('Unique w/full coverage: {}, good: {}, accuracy: {}'.format(len(self.words_checked), self.unique_good_all_ngbs, self.unique_good_all_ngbs/len(self.words_checked)))
...
relations2 = ResultCategory()
relations3 = ResultCategory()
relations4 = ResultCategory()
...
def rate_decision(decision, result_cat, output_file, fallback_decision=None, fallback_wordsets=[]):
...
for (sid, sent) in enumerate(sents):
    ...
    for (tid, token_data) in enumerate(sent):
        ...
        good1 = rate_decision(decision1, relations1, out1)
        good2 = rate_decision(decision2, relations2, out2, fallback_decision=decision1, fallback_wordsets=[relations1.words_checked])
        good3 = rate_decision(decision3, relations3, out3, fallback_decision=decision1, fallback_wordsets=[relations1.words_checked])
        rate_decision(decision4, relations4, out4, fallback_wordsets=[relations1.words_checked, relations2.words_checked, relations3.words_checked])
        if use_descriptions:
            rate_decision(decision5, unitdescs, outd)
